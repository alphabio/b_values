// b_path:: packages/b_declarations/src/properties/backdrop-filter/parser.ts
...
export function parseBackdropFilter(ast: csstree.Value): ParseResult<BackdropFilterIR> {
  const firstNode = ast.children.first;

  if (!firstNode) {
    return parseErr("backdrop-filter", createError("missing-value", "Empty value for backdrop-filter"));
  }
  ...
  return parseErr(
    "backdrop-filter",
    filterListResult.issues[0] ?? createError("invalid-value", "Invalid backdrop-filter value"),
  );
}
