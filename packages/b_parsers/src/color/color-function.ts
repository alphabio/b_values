...
export function parseColorFunction(node: csstree.FunctionNode): ParseResult<ColorFunction> {
  ...
  const c1Result = parseCssValueNodeEnhanced(values[1]);
  if (!c1Result.ok) return forwardParseErr<ColorFunction>(c1Result);

  const c2Result = parseCssValueNodeEnhanced(values[2]);
  if (!c2Result.ok) return forwardParseErr<ColorFunction>(c2Result);

  const c3Result = parseCssValueNodeEnhanced(values[3]);
  ...
  if (values.length === 5) {
    const alphaResult = parseCssValueNodeEnhanced(values[4]);
