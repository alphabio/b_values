// b_path:: packages/b_generators/src/gradient/__tests__/radial/color-stops.test.ts
import { describe, it, expect } from "vitest";
import type * as Type from "@b/types";
import * as Radial from "../../radial";

describe("Radial Gradient Generator - Color Stops", () => {
  describe("Basic Structures", () => {
    ...
    it("generates 3 stops without positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red, yellow, blue)");
      }
    });
    ...
    it("generates 5 stops without positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red, orange, yellow, green, blue)");
      }
    });
    ...
    it("generates 10 stops (stress test)", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe(`radial-gradient(${colors.join(", ")})`);
      }
    });
  });
  ...
  describe("Position Variations", () => {
    it("generates with percentage positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 0%, blue 100%)");
      }
    });
    ...
    it("generates with length positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 0px, blue 100px)");
      }
    });
    ...
    it("generates with mixed units", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 10px, blue 50%)");
      }
    });
    ...
    it("generates with double positions (color bands)", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 20% 40%, blue)");
      }
    });
    ...
    it("generates with all double positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 0% 25%, blue 25% 75%, green 75% 100%)");
      }
    });
    ...
    it("generates with mixed positions (some with, some without)", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red, yellow 30%, green, blue 90%)");
      }
    });
    ...
    it("generates with negative positions", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red -10%, blue 100%)");
      }
    });
    ...
    it("generates with positions over 100%", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red 0%, blue 150%)");
      }
    });
  });
  ...
  describe("Color Type Coverage", () => {
    it("generates with named colors", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red, transparent)");
      }
    });
    ...
    it("generates with hex colors", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(#ff0000, #0000ff)");
      }
    });
    ...
    it("generates with short hex colors", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(#f00, #00f)");
      }
    });
    ...
    it("generates with currentColor", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(currentcolor, blue)");
      }
    });
  });
  ...
  describe("Dynamic Values in Stops", () => {
    it("generates with var() in color", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(var(--color-1), var(--color-2))");
      }
    });
    ...
    it("generates with var() in position", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red var(--position), blue)");
      }
    });
    ...
    it("generates with calc() in position", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(red calc(50% - 10px), blue)");
      }
    });
    ...
    it("generates with all dynamic values", () => {
      ...
      expect(result.ok).toBe(true);
      if (result.ok) {
        expect(result.value).toBe("radial-gradient(var(--c1) var(--p1), var(--c2) var(--p2))");
      }
    });
  });
});
