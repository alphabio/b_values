...
**mask-_ mirrors background-_ exactly:**
...
export const parseBackgroundBlendMode = createMultiValueParser<BlendMode | CssValue, BackgroundBlendModeIR>({
...
export function parseMixBlendMode(valueNode: csstree.Value): ParseResult<MixBlendModeIR> {
  const nodes = Array.from(valueNode.children);
  const node = nodes[0];
...
1. **Layer-based properties** → Multi-value list (background-_, mask-_)
