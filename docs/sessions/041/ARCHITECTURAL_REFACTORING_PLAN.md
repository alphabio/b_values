    ...
    if (Ast.isFunctionNode(firstNode)) {
      ...
      if (funcName.includes('gradient')) {
        ...
        if (gradientResult.ok) {
          layerResults.push(parseOk({
            kind: 'gradient',
            gradient: gradientResult.value
          }));
        } else {
          // Preserve partial value for multi-error handling
          const value = gradientResult.value
            ? { kind: 'gradient', gradient: gradientResult.value }
            : undefined;
          layerResults.push({
            ok: false,
  ...
  try {
    ...
    return parseErr(
      createError(
        "invalid-syntax",
        e.message,
        { location: extractLocationFromError(e, sourceText) }
      )
    );
  }
...
describe("Error Quality", () => {
  test("precise error for invalid calc operator", () => {
    const result = parseProperty(
      "background-image: linear-gradient(red calc(50% & 20px), blue)"
    );
...
export function parseBackgroundImageFromString(
  css: string
): ParseResult<BackgroundImageIR> {
